...
// HELPER: DNS VERIFY
// ==========================================
async function isRobloxIP(ip) {
    const now = Date.now();
    if (ipCache[ip] && ipCache[ip].expiry > now) {
        return ipCache[ip].result;
    }

    try {
        const hostnames = await dns.reverse(ip);
        const result = hostnames.some(h => 
            h.endsWith('.roblox.com') || 
            h.endsWith('.amazonaws.com')
        );
        ipCache[ip] = { result, expiry: now + 60000 };
        return result;
    } catch {
        ipCache[ip] = { result: false, expiry: now + 60000 };
        return false;
    }
}
