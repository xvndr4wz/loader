...
const SETTINGS = {
    ...
    KEY_LIFETIME: 5000,   
    PLAIN_TEXT_RESP: "NGAPAIN MEMEK!", // Pesan untuk bot
    REAL_SCRIPT: `
        -- SCRIPT ASLI ANDA
        print("Ndraawz Security: Active!")
...
module.exports = async function(req, res) {
    ...
    // == GATEKEEPER KHUSUS EXECUTOR == \\
    // Roblox asli selalu mengirim User-Agent 'Roblox'
    // Dan biasanya mengirim header 'accept: */*' 
    // Bot fetcher/browser seringkali punya header 'accept' yang sangat panjang (html/json)
    const isRoblox = agent.includes("Roblox");
    const acceptHeader = req.headers['accept'] || "";

    // Proteksi: Jika bukan roblox ATAU diakses lewat browser (yang minta html)
    if (!isRoblox || acceptHeader.includes("text/html")) {
    ...
    try {
        if (currentStep > 0) {
            ...
            if (currentStep !== expectedStep) { delete sessions[id]; return res.status(403).send("SECURITY : WRONG STEP."); }

            if (session.used) { blacklist[ip] = true; return res.status(403).send("SECURITY : REPLAY."); }

            if (now - session.startTime > SETTINGS.SESSION_EXPIRY) { delete sessions[id]; return res.status(403).send("SECURITY : EXPIRED."); }

            if (session.nextKey !== key) { delete sessions[id]; return res.status(403).send("SECURITY : WRONG KEY."); }
        ...
        if (currentStep === 0) {
            ...
            // Output simpel satu baris tanpa header manual di Lua
            const luaScript = `-- Ndraawz Security\ntask.wait(${waitTime/1000})\nloadstring(game:HttpGet("${nextUrl}"))()`;
        ...
        if (sessions[id].currentIndex < SETTINGS.TOTAL_LAYERS - 1) {
            ...
            sessions[newID] = { ...session, nextKey, lastTime: now, keyCreatedAt: now, used: false };
            delete sessions[id]; 

            const nextUrl = "https://" + host + currentPath + "?" + nextStep + "." + newID + "." + nextKey;
            const luaScript = `-- Ndraawz Security ${session.currentIndex + 1}\ntask.wait(${waitTime/1000})\nloadstring(game:HttpGet("${nextUrl}"))()`;
            return res.status(200).send(luaScript);
        }
