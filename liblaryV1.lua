...
function Tab:AddDropdown(Configs)
    ...
    local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected do
        ...
        -- Initialize Selected based on MultiSelect mode
        if MultiSelect then
            Selected = {}
            if CheckFlag(Flag) then
                ...
                if type(OpDefault) == "table" then
                    for _, v in pairs(OpDefault) do
                        Selected[tostring(v)] = true
                    end
                elseif OpDefault then
                    Selected[tostring(OpDefault)] = true
                end
            end
        else
            if type(OpDefault) == "table" then
                Selected = CheckFlag(Flag) and GetFlag(Flag) or OpDefault[1]
            else
        ...
        local function UpdateSelected()
            if MultiSelect then
                for _,v in pairs(Options) do
                    local nodes = v.nodes
                    local Stats = Selected[v.Name] or false
    ...
    function Dropdown:Set(Val1, Clear)
        if type(Val1) == "table" then
            if DMultiSelect then
                ...
                -- Set new values
                for _, name in pairs(Val1) do
                    local nameStr = tostring(name)
                    if GetOptions()[nameStr] then
                        Selected[nameStr] = true
                    end
                end
                SetFlag(Flag, Selected)
                task.spawn(Callback, Selected)
        ...
        elseif type(Val1) == "function" then
            Callback = Val1
        elseif type(Val1) == "string" and not DMultiSelect then
            Selected = Val1
            SetFlag(Flag, Selected)
            task.spawn(Callback, Selected)
        end
